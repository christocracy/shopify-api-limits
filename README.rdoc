...
== Automatically aggregated resultsets when your limit is > 250
...
http://wiki.shopify.com/Retrieving_more_than_250_Products%2C_Orders_etc. which involves manually calculating the total & number of pages then iterating to make several API calls.  This gem encapsulates this behaviour allowing you to make what seems like a single call to the ShopifyAPI.  Simply set <b>:params => {:limit => false}</b>.  False as in, "no limit"
...
Without <b>{:limit => false}, the normal behaviour will operate (ie: just one request with 250 records returned):

  records = ShopifyAPI::Order.all
  puts records.count
  => 250
