...
http://bit.ly/kgwCRc which involves manually calculating the total & number of pages then iterating to make several API calls.  This gem encapsulates this behaviour allowing you to make what seems like a single call to the ShopifyAPI.  Simply set <b>:params => {:limit => false}</b>.  False as in, "no limit"
